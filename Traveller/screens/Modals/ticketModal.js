...
import { StyleSheet, Text, TouchableOpacity, Modal, View, TouchableWithoutFeedback, Image, FlatList, Dimensions, TextInput, SafeAreaView, LogBox } from "react-native";
        ...
        return (
            <SafeAreaView style={modalstyles.container}>
                ...
                <View style={modalstyles.top}>
                    ...
                    <Text style={{ color: "#FFF", fontSize: 20, fontFamily: font.text, marginLeft: 20 }}>{data.title}</Text>
                </View>
                <Text style={modalstyles.name}>{data.surname} {data.name}</Text>
                <Text style={modalstyles.destination} >Da{from} a{to}</Text>
                                ...
                                renderItem={({ item }) => (
                                    <View style={{ flexDirection: "row", justifyContent: "flex-start", alignItems: "center", width: Dimensions.get("window").width - 40, height: 50, borderBottomWidth: 1, borderBottomColor: "lightgray" }}>
                                        <Avatar
                                            size={40}
                                            autoColor
                                            labelStyle={{ fontFamily: font.text, fontSize: 20, color: "#FFF", fontWeight: "bold" }}
                                            label={item.name + " " + item.surname}
                                            style={{ marginRight: 10 }}
                                        />

                                        <Text style={{ fontFamily: font.text, fontSize: 20, color: "black" }}>{item.name} {item.surname}</Text>
                                        ...
                                        >
                                            <View style={(!item.sent) ? { borderRadius: 10, borderWidth: 1, backgroundColor: "#4900FF", paddingLeft: 10, paddingRight: 10, paddingTop: 5, paddingBottom: 5 } : { borderRadius: 10, borderWidth: 1, backgroundColor: "green", paddingLeft: 10, paddingRight: 10, paddingTop: 5, paddingBottom: 5 }} >
                                                <Text style={{ fontFamily: font.text, fontSize: 15, color: "white", textAlign: "center" }} >{(!item.sent) ? "Invia" : "Inviato"}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    </View>
                                )}
                            ...
                            <View style={ComponentStyles.editContent}>
                                ...
                                >
                                    <View style={{ borderBottomWidth: 1, borderBottomColor: "lightgray", paddingBottom: 10, flexDirection: "row", justifyContent: "flex-start", alignItems: "center" }} >
                                        ...
                                        <Text style={{ fontFamily: font.text, fontSize: 20, color: "black" }}>Condividi il biglietto</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={() => {
                                    ...
                                    >
                                        ...
                                        <Text style={{ fontFamily: font.text, fontSize: 20, color: "red" }}>Elimina biglietto</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                        </SafeAreaView>
