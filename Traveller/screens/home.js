...
export default function Home({ navigation }) {
  ...
  const verifyLog = async () => {
    ...
    if (
      data != null &&
      JSON.stringify(data) != '{"_h":0,"_i":0,"_j":null,"_k":null}' &&
      data != false &&
      data != ""
    ) {
      setOpenLogin(false);
      await SecureStore.setItemAsync("user-info", JSON.stringify(data));
      console.log(data)
      let notificationToken = await SecureStore.getItemAsync("notif-token");
      if (!notificationToken) {
        let notifToken = await registerForPushNotificationsAsync().then(
          (token) => {
  ...
  useEffect(() => {
    ...
    async function verifyUserData() {
      ...
      if (
        ...
        axios
          .get(serverLink + "api/user/info?username=" + data.username)
          .then(async (response) => {
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    Aggiungi un immagine di copertina al tuo viaggio, e rendilo
                    ancora più unico!
                  </Text>
                </View>
                <View
                ...
                >
                  ...
                  >
                    Invia le immagini ai partecipanti di un viaggio, e permetti
                    a loro di scaricarle, anche più insieme!
                  </Text>
                  <View
                    style={{
                      flexDirection: "row",
        ...
        <View style={styles.blue}>
          <View style={styles.content}>
            <TextInput
              ...
              style={serchData.length == 0 ? styles.input : styles.inputResult}
            ...
            {serchData.length == 0 && !searchLoading ? null : !searchLoading &&
              serchData.length > 0 ? (
              <View
                style={{
                  marginBottom: 10,
  ...
  if (Device.isDevice) {
    ...
    token = (
      await Notifications.getExpoPushTokenAsync({
        projectId: "51a2596b-2325-4af9-95fa-b2a4eb4f7558",
      })
    ).data;
    console.log(token);
  } else {
